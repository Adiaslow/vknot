...
**Definition 2.1** (Truncation Relation): For compounds $A$ and $B$ in a combinatorial library, $A \preceq B$ ($A$ is a truncation of $B$) if $A$ can be obtained from $B$ by replacing one or more building blocks with null.

**Theorem 2.1** (DAG Structure): The truncation relation $\preceq$ forms a directed acyclic graph.
...
**Definition 3.1** (Discrete Morse Function): For discrete signal $s: \mathbb{Z} \to \mathbb{R}$, position $i$ is a critical point if:
...
**Definition 3.2** (Poisson Z-score): For observed count $x$ with background rate $\lambda$:
...
**Definition 4.1** ($L_0$): The minimal element of the DAG, representing the fully-null compound.

**Theorem 4.1** (NULL Peak Inheritance): All compounds must contain peaks corresponding to $L_0$'s peaks.
...
**Theorem 4.2** (Optimal Assignment): The Hungarian algorithm finds the globally optimal peak assignment minimizing total retention time deviation in $O(n^3)$.
