...
Therefore, the truncation relation forms a DAG. $\blacksquare$
...
The convergence is not a cycle because edges only point toward simpler structures. $\blacksquare$
...
Therefore $\sim$ is an equivalence relation, partitioning the space into disjoint classes. $\blacksquare$
...
Critical points are preserved. $\blacksquare$
...
$P(Z > Z_t) = 1 - \Phi(Z_t) = \Phi(-Z_t)$. $\blacksquare$
...
- No true peaks are missed if they exceed noise threshold $\blacksquare$
...
Therefore, $L_0$'s peaks appear in every chromatogram. $\blacksquare$
...
The solution minimizes $\sum C[i, \pi(i)]$ over all permutations $\pi$. $\blacksquare$
...
Therefore RT(C) > RT(T). $\blacksquare$
...
Edge construction: $O(V^2)$ worst case, but sparse in practice. $\blacksquare$
...
- Total: $O(n)$ per signal $\blacksquare$
...
- Total: $O(V \times p^3)$ $\blacksquare$
...
The DAG structure ensures no circular dependencies. $\blacksquare$
...
- Constraints determine unique classification $\blacksquare$
...
- Never produces provably incorrect result $\blacksquare$
...
Together, these constrain the solution space to enable validation. $\blacksquare$
