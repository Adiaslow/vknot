...
Both cells die due to competitive pressure (Rule 4), permanently removing mass from the system. No local rearrangement can recover this loss. This breaks time-reversibility and prevents conservation laws. $\blacksquare$
...
This creates a flow from high to low pressure regions, analogous to fluid dynamics. The boundary between territories acts as an interface with surface tension proportional to the pressure differential. $\blacksquare$
...
- Pattern structure is preserved $\blacksquare$
...
A pure pattern at a contested boundary faces pressure $P \neq 0$, causing erosion. The chimeric pattern has $P = 0$ everywhere, achieving superior stability. $\blacksquare$
...
Therefore, no pure strategy can be a best response to itself. $\blacksquare$
...
By Nash's theorem, a mixed strategy equilibrium exists. $\blacksquare$
...
Quadratic growth dominates linear strategies asymptotically. $\blacksquare$
...
Honey farms are twice as space-efficient when defended. $\blacksquare$
...
Since Generalized Geography is PSPACE-complete, so is optimal Cloth of Gold play. $\blacksquare$
...
No single strategy dominates all others. $\blacksquare$
