...
$$
\begin{aligned}
C(b) &= \frac{|U|\log|U|}{b} \times \text{setup\_cost} + |U|\log|U| \times \text{per\_sample\_cost} \\
&= |U|\log|U| \times \left(\frac{\beta}{b} + 1\right) \times \text{per\_sample\_cost}
\end{aligned}
$$

Minimizing with respect to b:

$$\frac{dC}{db} = -|U|\log|U| \times \frac{\beta}{b^2} \times \text{per\_sample\_cost} = 0$$
...
$$\text{Var\_reduction} = \frac{(\sum|F_i||U_i|)^2}{\sum|F_i|^2 \times |U_i|}$$
