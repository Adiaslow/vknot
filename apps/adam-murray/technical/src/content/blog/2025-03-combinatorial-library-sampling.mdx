...
import PhaseTransitionExplorer from '../../components/interactive/PhaseTransitionExplorer.tsx';
import TemporalOptimizationFlow from '../../components/interactive/TemporalOptimizationFlow.tsx';
...
<PhaseTransitionExplorer client:load />
...
A combinatorial library forms a quotient space $(S, U, \pi)$ where:
...
The redundancy factor $R = |S|/|U|$ measures average equivalence class size.
...
$$P(|\{\pi(s_1), \ldots, \pi(s_n)\}| \geq C|U|) \geq 1 - \delta$$
...
- $N$ = number of variable positions in combinatorial synthesis
- $B_i$ = number of building block choices at position $i \in \{1,\ldots,N\}$
- $|U|_{max} = \prod_i B_i$ = maximum diversity (if all combinations were unique)
- $\rho = |U|_{max}/|U|$ = chemical redundancy (ratio of possible to actual unique entities)
- $R = |S|/|U|$ = synthesis redundancy (average synthesis paths per entity)
...
For any sampling strategy with $n$ samples and unknown $|U|$:

$$E[\text{samples needed}] \geq \frac{|U|H(P)}{\log|U|}$$
...
The information gain from sample $s_n$ given previous samples $S_{n-1}$:

$$I(s_n) = H(U|S_{n-1}) - H(U|S_{n-1} \cup \{s_n\})$$

### Theorem 3.2 (Logarithmic Advantage of Adaptivity)

For uniform sampling:

$$E[I(s_n)] = O\left(\frac{|U| - \text{observed}}{|U|} \times \log|U|\right)$$

For optimal adaptive sampling:

$$E[I(s_n)] = O(\log(|U| - \text{observed}))$$
...
Adaptive sampling requires $O(|U|)$ samples versus $O(|U| \log |U|)$ for uniform sampling.

### 3.3 Partial Information Regimes

### Definition 3.2 (Knowledge Level)

$\kappa \in [0,1]$ = fraction of quotient structure known a priori, where $\kappa=0$ means no knowledge and $\kappa=1$ means complete knowledge.

### Theorem 3.3 (Sample Complexity with Partial Information)

Given knowledge level $\kappa$ about quotient structure:

$$n^*(\kappa) = \frac{|U|(\log|U| + (1-\kappa)\log R)}{1 + \kappa(R-1)/R}$$
...
$n$ = number of samples drawn from synthesis space $S$.

### Definition 4.2 (Sampling Fraction)

$\alpha = n/|S|$ = fraction of synthesis space sampled.

### Definition 4.3 (Coverage Function)

$C(\alpha) = E[|\text{observed unique}|/|U|]$ = expected fraction of unique entities discovered.

### Definition 4.4 (Coverage Tolerance)

$\varepsilon \in (0,1)$ = maximum acceptable fraction of uncovered entities. Target coverage is $1-\varepsilon$.

### Definition 4.5 (Confidence Parameter)

$\delta \in (0,1)$ = maximum acceptable failure probability. We seek $P(\text{success}) \geq 1-\delta$.

### Definition 4.6 (Critical Sampling Fraction)

The critical sampling fraction $\alpha_c(\varepsilon) = \frac{\log(1/\varepsilon)}{R}$ is the threshold where phase transition occurs.

### Theorem 4.1 (Sharp Phase Transition)

For combinatorial libraries with redundancy $R = |S|/|U|$, the probability of achieving coverage $1-\varepsilon$ exhibits a sharp transition at $\alpha_c$:

$$
\lim_{|U| \to \infty} P(C(\alpha) > 1-\varepsilon) = \begin{cases}
0 & \text{if } \alpha < \alpha_c(\varepsilon) \\
1 & \text{if } \alpha > \alpha_c(\varepsilon)
\end{cases}
$$
...
$$P(\text{uncovered} < \varepsilon|U|/2) \leq \exp(-\varepsilon|U|/8)$$

For $\alpha > \alpha_c$: Expected uncovered is $< \varepsilon|U|$.
Similarly:

$$P(\text{uncovered} > 2\varepsilon|U|) \leq \exp(-\varepsilon|U|/8)$$
...
$$
\begin{aligned}
\alpha_1 &= 1/|S| && \text{(first discovery)} \\
\alpha_2 &= \frac{\log|B|}{R_B} && \text{(building block coverage)} \\
\alpha_3 &= \frac{\log|F|}{R_F} && \text{(fragment coverage)} \\
\alpha_4 &= \frac{\log|U|}{R} && \text{(molecular coverage)}
\end{aligned}
$$
...
$$\alpha_{\text{perc}} = \frac{\log d}{d}$$

Below $\alpha_{\text{perc}}$: Isolated clusters of discovered chemicals.
Above $\alpha_{\text{perc}}$: Giant component containing $\Theta(|U|)$ chemicals forms.

Proof sketch:
Maps to Erdős-Rényi random graph with edge probability $p = \alpha R|U|/|S| = \alpha$ (since $R = |S|/|U|$). Giant component emerges when $p > \log d/d$.
...
- $K$ = number of experimental modalities (e.g., assay types)
- $Q_k$ = cost per sample for modality $k \in \{1,\ldots,K\}$
- $I_k$ = information rate (bits per sample) for modality $k$
- $C_k(n)$ = coverage achieved by $n$ samples in modality $k$
- $n_k$ = number of samples allocated to modality $k$

**Problem** (Multi-Modal Resource Allocation): Given $K$ experimental modalities with costs $Q_k$, information rates $I_k$, and coverage functions $C_k(n)$, maximize coverage subject to budget $B$.
...
$$
C(n_1,\ldots,n_i+1,\ldots) - C(n_1,\ldots,n_i,\ldots) \geq C(n_1,\ldots,n_i+1,\ldots,n_j+1,\ldots) - C(n_1,\ldots,n_i,\ldots,n_j+1,\ldots)
$$
...
Greedy allocation achieves $(1-1/e)$ approximation of optimal coverage.
...
$$P(r) \propto \sqrt{\text{value}(r) \times \text{uncertainty}(r)}$$
...
- $\beta$ = setup_cost/per_sample_cost = ratio of fixed setup cost to variable per-sample cost

### Theorem 5.3 (Optimal Batch Size)

The cost-minimizing batch size is:

$$b^* = \sqrt{\beta \times |U| \times \log|U|}$$

independent of $|S|$ when $R \gg \log|U|$.
...
$$
\begin{aligned}
C(b) &= \frac{|U|\log|U|}{b} \times \text{setup:cost} + |U|\log|U| \times \text{per:sample:cost} \\
&= |U|\log|U| \times \left(\frac{\beta}{b} + 1\right) \times \text{per:sample:cost}
\end{aligned}
$$

Minimizing with respect to b:

$$\frac{dC}{db} = -|U|\log|U| \times \frac{\beta}{b^2} \times \text{per:sample:cost} = 0$$
...
<TemporalOptimizationFlow client:load />

### Definition 5.3 (Temporal Variables)

- $t$ = time from start of experiment
- $\lambda$ = exponential decay rate of sample quality
- $Q(t) = Q_0\exp(-\lambda t)$ = sample quality at time $t$
- $I(t) = I_0\exp(-\lambda t)$ = information gained per sample at time $t$
- $n(t)$ = sampling rate (samples per unit time) at time $t$
- $C$ = cost per sample (constant)
- $B$ = total budget constraint
- $\mu$ = Lagrange multiplier for budget constraint

**Problem**: Given sample quality decay $Q(t) = Q_0\exp(-\lambda t)$, optimize sampling schedule.

### Theorem 5.4 (Front-Loading Principle)

Optimal sampling rate is:

$$n^*(t) \propto \exp(-\lambda t/2)$$

### Proof

Information per sample at time $t$: $I(t) = I_0\exp(-\lambda t)$.
Cost per sample: constant $C$.
Lagrangian for maximizing $\int I(t)n(t)dt$ subject to $\int Cn(t)dt \leq B$:

$$L = \int[I_0\exp(-\lambda t)n(t) - \mu Cn(t)]dt$$

First-order condition: $I_0\exp(-\lambda t) = \mu C$.
...
$$\text{Var:reduction} = \frac{(\sum|F_i||U_i|)^2}{\sum|F_i|^2 \times |U_i|}$$
...
- $|S| = 10^9$ sequences
- $|U| = 10^4$ unique molecules
- $R = 10^5$ redundancy

**Complete enumeration**: $10^9 \times 261 = 2.61 \times 10^{11}$ operations
**Optimal sampling**: $10^4 \times \log(10^4) \times 261 \approx 2.4 \times 10^8$ operations
...
$$\alpha_c = \frac{\log(10^4)}{10^5 \times 10^4/10^9} = 9.2 \times 10^{-4}$$
...
For n samples with $n = |U|(\log|U| + \log(1/\delta))$:

$$P(\text{complete coverage}) \geq 1 - \delta$$

For $99.9\%$ confidence ($\delta = 0.001$)

$$n = 10^4(\log(10^4) + \log(1000)) = 10^4 \times 16.1 \approx 161{,}000$$

Still $6000\times$ fewer than complete enumeration.
