...
Rearranging gives the bound. $\blacksquare$
...
Adaptive sampling can target highest-uncertainty regions, guaranteeing discovery until coverage is complete, providing $\log(\text{remaining})$ bits deterministically. $\blacksquare$
...
- Combines multiplicatively due to information independence $\blacksquare$

## 4. Phase Transitions

### 4.1 Coverage Phase Transition

### Definition 4.1 (Sampling Fraction)

$\alpha = n/|S|$ where $n$ is number of samples.

### Definition 4.2 (Coverage Function)

$C(\alpha) = E[|\text{observed unique}|/|U|]$.

### Theorem 4.1 (Sharp Phase Transition)

For combinatorial libraries with redundancy $R = |S|/|U|$, there exists critical $\alpha_c$ such that:
...
As $|U| \to \infty$, these probabilities $\to 0$, giving sharp transition. $\blacksquare$
...
\alpha_2 &= \log|B|/(R_B \times |B|/|S|) && \text{(building block coverage)} \\
\alpha_3 &= \log|F|/(R_F \times |F|/|S|) && \text{(fragment coverage)} \\
\alpha_4 &= \log|U|/(R \times |U|/|S|) && \text{(molecular coverage)}
\end{aligned}
$$
...
$$\alpha_{\text{perc}} = \frac{\log d}{d \times R \times |U|/|S|}$$
...
Proof sketch:
Maps to Erdős-Rényi random graph with edge probability $p = \alpha R|U|/|S|$. Giant component emerges when $p > \log d/d$. $\blacksquare$
...
Each additional sample has probability $p$ of discovering new entity. With more samples, fewer entities remain undiscovered, so $p$ decreases. Therefore marginal benefit decreases. $\blacksquare$
...
Uses upper confidence bound (UCB) framework. Optimal exploration bonus is proportional to √(uncertainty), while exploitation weight is proportional to value. Product gives optimal trade-off. $\blacksquare$

### 5.3 Batch Optimization

### Definition 5.2 (Batch Economy)

β = setup_cost/per_sample_cost.
...
Since coverage requirement $|U|\log|U|$ is independent of $|S|$ under high redundancy, so is $b^*$. $\blacksquare$
...
Solving for $n(t)$ gives the result. $\blacksquare$
