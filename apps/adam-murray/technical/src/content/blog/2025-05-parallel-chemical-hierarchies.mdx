...
Therefore, parallel decomposition preserves at least as much information. $\blacksquare$
...
The finer partition captures more structural information. $\blacksquare$
...
For cyclic peptides, $\text{diameter}(G) \approx n/2$ for $n$ residues, making attention significantly more efficient for long-range interactions. $\blacksquare$
...
Therefore, the parallel hierarchy has higher representational capacity. $\blacksquare$
...
The parallel structure provides gradient flow redundancy. $\blacksquare$
...
The parallel structure spans the space of permeability features. $\blacksquare$
