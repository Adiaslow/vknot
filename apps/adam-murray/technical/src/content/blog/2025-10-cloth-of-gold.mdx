...
```
N_A[w_{i,j}] = Σ_{k=i-1}^{i+1} Σ_{l=j-1}^{j+1} 𝟙[w_{k,l} = a], (k,l) ≠ (i,j)
```

**Player B neighbors**:

```
N_B[w_{i,j}] = Σ_{k=i-1}^{i+1} Σ_{l=j-1}^{j+1} 𝟙[w_{k,l} = b], (k,l) ≠ (i,j)
```
...
```
w_{i,j}(t+1) = 0 if N[w_{i,j}] < 2 or N[w_{i,j}] > 3
```
...
```
w_{i,j}(t+1) = a if:
  - (w_{i,j} ∈ {a,c} and N[w_{i,j}] ∈ {2,3}) OR (w_{i,j} = 0 and N[w_{i,j}] = 3)
  - AND N_A[w_{i,j}] > N_B[w_{i,j}]
```
...
```
w_{i,j}(t+1) = b if:
  - (w_{i,j} ∈ {b,c} and N[w_{i,j}] ∈ {2,3}) OR (w_{i,j} = 0 and N[w_{i,j}] = 3)
  - AND N_B[w_{i,j}] > N_A[w_{i,j}]
```

**Rule 4** (Stalemate preservation):

```
w_{i,j}(t+1) = w_{i,j}(t) if:
  - N_A[w_{i,j}] = N_B[w_{i,j}]
  - AND 2 ≤ N[w_{i,j}] ≤ 3
```
...
```
Territory(c) = {
    A's territory:    if I_A(c) > 1.0 AND I_A(c) > I_B(c)
    B's territory:    if I_B(c) > 1.0 AND I_B(c) > I_A(c)
    Contested:        if I_A(c) > 1.0 AND I_B(c) > 1.0 AND |I_A(c) - I_B(c)| < ε
    Neutral:          if max(I_A(c), I_B(c)) < 1.0
}
```
...
2. All boundary cells have ≥ 3 friendly neighbors
3. No boundary cell has majority enemy neighbors

**Proof:**
Necessity: If any condition fails:

1. Interior cells with ≠ 2-3 neighbors die (standard rules)
...
A pure pattern at a contested boundary faces pressure P ≠ 0, causing erosion. The chimeric pattern has P = 0 everywhere, achieving superior stability. ∎
...
**Definition 4.2** (Boundary Treatment): Cells outside the m×n board are treated as permanently empty (state 0) for all calculations.
...
**Theorem 4.1** (No Pure Strategy Equilibrium): For non-trivial boards (m,n ≥ 10), no pure strategy Nash equilibrium exists.
...
The chaotic evolution (Lyapunov exponent λ > 0) means small placement changes cascade unpredictably, preventing stable best responses for any pure strategy.
...
```
H(opponent_placement | my_placement) = log₂(C(available_cells, 10))
```

For a 100×100 board with 50% available cells:

```
H ≈ log₂(C(5000, 10)) ≈ 33 bits
```
...
```
R(t+1) = R(t) + 10 + Σ_{p ∈ patterns(t)} value(p)
```
...
```
R(t) = 10t + 8·Σ_{i=1}^t n(i)
```

If n(t) grows linearly: n(t) = αt

```
R(t) = 10t + 8α·t(t+1)/2 = O(t²)
```
...
**Theorem 5.2** (Packing Density): Honey farms achieve 2× better resource density than distributed beehives.

**Proof:**
Beehive + 2 blocks requires ≈25 cells, generates 1 unit:

```
Density_distributed = A/25 units/round
```

Honey farm + 8 blocks requires ≈100 cells, generates 8 units:

```
Density_honey = 8A/100 = 0.08A units/round
```
...
**Definition 6.2** (Density): ρ = (|A| + |B|)/(mn)

**Theorem 6.2** (Phase Structure): The system exhibits distinct phases:

```
ρ ∈ [0, 0.1): Sparse - patterns evolve independently
ρ ∈ [0.1, 0.3): Percolation - local interactions begin
ρ ∈ [0.3, 0.5): Competition - territorial warfare
ρ ∈ [0.5, 1.0]: Overcrowded - mutual destruction
```

**Critical point**: ρ_c ≈ 0.35 where largest sustainable populations exist.
...
3. Winning Geography strategy ↔ winning Cloth of Gold strategy
...
- Naive: O(mn·|pattern_library|)
- With hashing: O(mn) expected time
- Via convolution: O(mn·log(mn)) using FFT
...
- Sparse boards (early game): O(k) ≪ O(mn)
...
```
Economic → Defensive → Aggressive → Economic
```
...
**Theorem 8.2** (Convergence): As t → ∞, one of four outcomes occurs:
...
```
E[duration] = 52·P(no early victory) + Σ_{t<52} t·P(victory at t)
```

With balanced players: E[duration] ≈ 35-45 rounds.
